...
__all__ = [
    ...
    "RXNBERTMinhashFingerprintGenerator",
...
class RXNBERTFingerprintGenerator(FingerprintGenerator):
    ...
    def __init__(
        self, model: BertModel, tokenizer: SmilesTokenizer, force_no_cuda=False
    ):
    ...
    def convert(self, rxn_smiles: str):
        ...
        """
        bert_inputs = self.tokenizer.encode_plus(
            rxn_smiles,
...
def get_default_model_and_tokenizer(model="bert_ft", force_no_cuda=False):
    ...
    model_path = str(_pkg_dir / "models" / "transformers" / model)

    tokenizer_vocab_path = str(
        _pkg_dir / "models" / "transformers" / model / "vocab.txt"
    )
    device = torch.device(
        "cuda" if (torch.cuda.is_available() and not force_no_cuda) else "cpu"
    )
...
def generate_fingerprints(
    rxns: List[str], fingerprint_generator: FingerprintGenerator, batch_size=1
) -> np.array:
    fps = []
